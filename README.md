...
For their investigation, Nalisnick et al. train different generative models, representing species of VAEs, autoregressive models and flow-based generative models on several datasets, and testing them on unrelated datasets. They find that all these models can assign higher probabiltiy distributions to unrealted datasets the models were not trained on and than those they were trained on. This suggest that generative models would, for instance, when confronted with anomalies, predict with high confidence, not detecting such out-distribution data. The example also demonstrates that a thorough analysis of the observed phenomena is necessary for application of generative models in industry. 
...
We have shown that their research is reproducible, to the extent possible to us. Their conclusion however is less based on evidence, something we are going to analyze in the following. Their explanation for the failure of generative models is the confused models show similiar their distributions favoring some models. For this explaniation, they however assume that the underlying distributions of different datasets are normal, a strong assumption. In the following, we are going to question this assumption by fitting other distributions and representations. 

More spefically, we are going to fit different, non-gaussian distributions first, such as Cauchy-distribution and F-distribution. Additionally, we are going to analyze the datsaset in a reduced, latent space representation to investigate if the different datasets still exhibt such strong similarities when compressed. For that, we apply PCA, a traditional dimenionsality reduction. Additionally, we will train an autoencoder to obtain a learned representation. Simillarily, we will also use the reduced space representation of a discriminator trained on the datasets.
